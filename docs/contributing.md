...
4. **Configure environment variables**:
   ...
   **Note**: We use Convex Cloud. If you are migrating from legacy self-hosted infrastructure, follow [CONVEX_CLOUD_MIGRATION.md](CONVEX_CLOUD_MIGRATION.md).
...
5. **Start Convex backend**:
   ...
   This will:
   - Connect to your Convex Cloud deployment from `CONVEX_DEPLOYMENT`
...
- `EXPO_PUBLIC_CONVEX_URL` - Convex Cloud deployment URL used by the app client

**Backend** (`backend/.env.local`):

- `CONVEX_DEPLOYMENT` - Convex Cloud deployment slug (generated by `npx convex dev`)
